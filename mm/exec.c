...
PUBLIC int do_exec()
{
	...
	//printl("\n{MM} MM::do_exec()::pathname:%s\n",pathname);

	/* get the file size */
	struct fat16_stat_t s;
	int ret = stat(pathname, &s);
	//printl("{MM} MM::do_exec()::stat()::ret :%d\n",ret);
	if (ret != 0) {
	...
	/* read the file */
	int fd_stdin  = open("dev_tty0.ch", O_RDWR);
	assert(fd_stdin  == 0);
	int fd_stdout = open("dev_tty0.ch", O_RDWR);
	assert(fd_stdout == 1);
	int fd = open(pathname, O_RDWR);
	//printl("{MM} MM::do_exec()::open()::fd :%d\n",fd);
	if (fd == -1)
		return -1;
	assert(s.size < MMBUF_SIZE);
	ret= read(fd, mmbuf, s.size);
	//printl("{MM} MM::do_exec()::read()::size :%d\n",ret);
	ret= close(fd);
	//printl("{MM} MM::do_exec()::close()::ret :%d\n",ret);
